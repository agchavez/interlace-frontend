...
import {
  AddDetalleBody,
  AddDetalleData,
  AddDetallePalletData,
  AddDetallePalletResponse,
  AddOutProductBody,
  AddOutProductData,
  CreateTrackingBody,
  Tracker,
...
export const getOpenTrackings = (): AppThunk => async (dispatch, getState) => {
  try {
    ...
    const seguimientos = data.map((tracker) =>
...
export const createTracking =
  (body: CreateTrackingBody): AppThunk =>
  async (dispatch, getState) => {
    try {
      dispatch(setLoading(true));
      const { token } = getState().auth;
      const { data: tracker } = await backendApi.post<
        Tracker,
...
export const chanceStatusTracking =
  (
  ...
  async (dispatch, getState) => {
    try {
      dispatch(setLoading(true));
      const { token } = getState().auth;
      await backendApi.patch<Tracker, AxiosResponse<Tracker>>(
        `/tracker/${trackerId}/`,
        { status },
        {
          headers: {
      ...
      toast.success("Seguimiento actualizado", {
        description:
          status === "PENDING"
            ? "El seguimiento se movio a pendientes"
            : "El seguimiento se encuentra ahora en atención",
      });
...
export const removeTracking =
  (indexSeguimiento: number, trackingId: number): AppThunk =>
  async (dispatch, getState) => {
    try {
      dispatch(setLoading(true));
      const { token } = getState().auth;
      const { status } = await backendApi.delete<
        Tracker,
        AxiosResponse<Tracker>
      >(`/tracker/${trackingId}/`, {
        headers: {
      ...
      if (status === 204) dispatch(removeSeguimiento(indexSeguimiento));
...
export const addDetalle =
  (index: number, data: AddDetalleData): AppThunk =>
  async (dispatch, getState) => {
    try {
      dispatch(setLoading(true));
      const { token } = getState().auth;
      ...
      const detalle = seguimiento.detalles.find(
        (detalle) => detalle.sap_code === data.product.sap_code
      );
      ...
      const { data: detail } = await backendApi.post<
        TrackerDetailResponse,
        AxiosResponse<TrackerDetailResponse>
      >(`/tracker-detail/`, body, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
...
export const removeDetalle =
  (segIdx: number, detalleIdx: number, id: number): AppThunk =>
  async (dispatch, getState) => {
    try {
      dispatch(setLoading(true));
      const { token } = getState().auth;
...
export const addDetallePallet =
  (indexSeg: number, indexDet: number, data: AddDetallePalletData): AppThunk =>
  async (dispatch, getState) => {
    try {
      dispatch(setLoading(true));
      const { token } = getState().auth;
      ...
      const { data: detailProduct } = await backendApi.post<
        TrackerProductDetail,
        AxiosResponse<TrackerProductDetail>
      >(`/tracker-detail-product/`, data, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
...
export const updateDetallePallet =
  (
    indexSeg: number,
    indexDet: number,
    indexPallet: number,
    data: UpdateDetallePalletData
  ): AppThunk =>
  async (dispatch, getState) => {
    try {
      dispatch(setLoading(true));
      ...
      const seguimiento = seguimientos[indexSeg];
      if (!seguimiento) return;
      const { data: detailProduct } = await backendApi.patch<
        TrackerProductDetail,
        AxiosResponse<TrackerProductDetail>
      >(
        `/tracker-detail-product/${data.id}/`,
        {
          ...data,
...
export const removeDetallePallet =
  (
    indexSeg: number,
    indexDet: number,
    indexPallet: number,
  ...
  async (dispatch, getState) => {
    try {
      ...
      const { status } = await backendApi.delete<
        AddDetallePalletResponse,
        AxiosResponse<AddDetallePalletResponse>
      >(`/tracker-detail-product/${id}/`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
...
export const completeTracker = (): AppThunk => async (dispatch, getState) => {
  try {
    ...
    const { status } = await backendApi.post(
      `/tracker/${idSeguimiento}/complete/`,
      {},
      {
        headers: {
          Authorization: `Bearer ${token}`,
...
export const addOutProduct =
  (indexSeguimiento: number, data: AddOutProductData): AppThunk =>
  async (dispatch, getState) => {
    try {
      dispatch(setLoading(true));
      const { token } = getState().auth;
      ...
      const seguimiento = seguimientos[seguimeintoActual];
      if (
        seguimiento.detallesSalida?.find(
          (d) => d.sap_code === data.product.sap_code
        )
      ) {
...
export const removeOutProduct =
  (indexSeguimiento: number, detalle: DetalleCargaSalida): AppThunk =>
  async (dispatch, getState) => {
    try {
      dispatch(setLoading(true));
      const { token } = getState().auth;
      const { status } = await backendApi.delete(
        `/tracker-detail-output/${detalle.idDetalle}/`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
  ...
  async (dispatch, getState) => {
    try {
      ...
      const { status, data } = await backendApi.patch<
        Tracker,
        AxiosResponse<Tracker>
      >(`/tracker/${trackerId}/upload-file/`, formData, config);
      if (status < 400) {
        dispatch(
          updateSeguimiento({
            index: indexSeguimiento,
...
export const downloadFile =
  (trackerId: number, onStopLoading?: () => void): AppThunk =>
  async (dispatch, getState) => {
    try {
      dispatch(setLoading(true));
      const { token } = getState().auth;
...
export const removeFile =
  ...
  async (dispatch, getState) => {
    try {
      dispatch(setLoading(true));
      const { token } = getState().auth;
      const { data, status } = await backendApi.delete<
        Tracker,
        AxiosResponse<Tracker>
      >(`/tracker/${trackerId}/delete-file/`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
...
export const parseTrackerSeguimiento = (tracker: Tracker): Seguimiento => {
  const seguimiento: Seguimiento = {
    ...
    timeStart:
      tracker.input_date !== null
        ? new Date(tracker.input_date || 0).toISOString()
        : null,
    timeEnd:
      tracker.output_date !== null
        ? new Date(tracker.output_date || 0).toISOString()
        : null,
    ...
    detallesSalida: tracker.tracker_detail_output.map((out) =>
...
const parseDetail = (tracker_detail: TrackerDetailResponse): DetalleCarga => {
  const seguimiento_detalle: DetalleCarga = {
    ...
    history: tracker_detail.tracker_product_detail
      .map((det) => parseProductDetail(det))
      .reverse(),
    ...
    days_not_accept_product:
      tracker_detail.product_data.days_not_accept_product,
...
const parseProductDetail = (
  tracker_detail: TrackerProductDetail
): DetalleCargaPalet => {
...
const parseOutputDetailSalida = (
  tracker_detail: TrackerDeailOutput
): DetalleCargaSalida => {
