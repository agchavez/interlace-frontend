...
const STATUS_COLOR: Record<string, 'default' | 'primary' | 'success' | 'warning' | 'info'> = {
    PENDING_COUNT: 'default',
    COUNTING: 'warning',
    COUNTED: 'success',
};

export default function CounterPautaCard({ pauta, onClick }: Props) {
    const theme = useTheme();
    const elapsed = useElapsed(pauta.status === 'COUNTING' ? pauta.last_status_change : null);
    const isDone = pauta.status === 'COUNTED';
