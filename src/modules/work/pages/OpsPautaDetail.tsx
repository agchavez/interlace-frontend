...
export default function OpsPautaDetail() {
    ...
    const [takeAsOps, { isLoading: validating }] = useTakeAsOpsMutation();
    ...
    const handleValidate = async () => {
        if (!pauta) return;
        try {
            await takeAsOps(pauta.id).unwrap();
            showSnack('Validación de operaciones registrada');
    ...
    const canValidate = pauta.status === 'CHECKOUT_SECURITY';
    const isValidated = ['CHECKOUT_OPS', 'DISPATCHED', 'CLOSED'].includes(pauta.status);
        ...
        <Box sx={{ height: 'calc(100dvh - 60px)', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
                    ...
                    {canValidate && (
                        <Paper variant="outlined" sx={{ p: 2, mb: 2, borderRadius: 3 }}>
                            ...
                            <Typography variant="body2" color="text.secondary" sx={{ mb: 1.5 }}>
                                Esta validación es opcional. Si todo está correcto puedes despachar directamente.
                            </Typography>
                            <Button
                                fullWidth
                                variant="outlined"
                                color="warning"
                                endIcon={<OpenIcon />}
                                onClick={() => navigate(`/truck-cycle/verify/${pauta.id}?phase=CHECKOUT_OPS`)}
                            >
            ...
            {canValidate && (
                <Box sx={{ position: 'sticky', bottom: 0, zIndex: 5, bgcolor: 'background.paper', borderTop: 1, borderColor: 'divider' }}>
                    <Container maxWidth="md" sx={{ py: 2, display: 'flex', flexDirection: 'column', gap: 1.25 }}>
                        <SwipeToConfirm
                            label="Desliza para validar ops"
                        ...
                        <Button
                            ...
                            variant="text"
                            color="primary"
                            startIcon={dispatching ? <CircularProgress size={16} /> : <DispatchIcon />}
                            onClick={handleDispatchDirect}
                            disabled={dispatching}
                            sx={{ py: 1.25, fontWeight: 700 }}
                        >
                            Saltar ops y despachar directo
                        </Button>
                    </Container>
                </Box>
            )}
